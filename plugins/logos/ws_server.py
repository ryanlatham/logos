...
def sign_hello(
    ...
    *,
    ...
    return hmac.new(
        secret.strip().encode("utf-8"), message.encode("utf-8"), hashlib.sha256
    ).hexdigest()
...
class LogosWebSocketServer:
    ...
    def __init__(
        ...
        *,
        ...
        self.enc_mode = (
            enc_mode
            if enc_mode in (ENC_MODE_NEGOTIATE, ENC_MODE_REQUIRED, ENC_MODE_OFF)
            else ENC_MODE_NEGOTIATE
        )
    ...
    async def _send_locked(
        self, websocket: Any, meta: dict[str, Any] | None, frame: dict[str, Any]
    ) -> None:
                ...
                try:
                    ...
                    if not authenticated:
                        ...
                        if envelope.type != "hello":
                            ...
                            continue
                        authenticated_result, auth_reason, auth_message, auth_details, enc_ctx = (
                        ...
                        if not authenticated_result:
                            ...
                            await websocket.send(serialize_frame(auth_error))
                        ...
                        client_config = (
                            self.adapter.client_config_payload()
                            if hasattr(self.adapter, "client_config_payload")
                            else {}
                        )
                        ...
                        if (
                            enc_ctx
                            and self.enc_mode != ENC_MODE_OFF
                            and enc_ctx.get("enc_client_nonce_bytes")
                        ):
                            ...
                            if selected_aead is not None:
                                ...
                                enc_response = {
                                    "aead": selected_aead,
                                    "mode": "hkdf-v1",
                                    "enc_server_nonce": base64.b64encode(server_nonce).decode(
                                        "ascii"
                                    ),
                                }
                        ...
                        hello_payload: dict[str, Any] = {
                            ...
                            "server": "logos",
                            "auth": "hmac-sha256",
                            "credential_scope": auth_details.get(
                    ...
                    if crypto is not None and is_encrypted_payload(envelope.payload):
                        try:
                            envelope = replace(
                                envelope,
                                payload=crypto.open_payload(envelope.to_dict(), envelope.payload),
                            )
                        except CryptoError:
                            await self._send_locked(
                                ...
                                meta,
                                error_frame(
                                    "decrypt_failed", "Logos failed to open an encrypted frame."
                                ),
                            )
                            continue
                        ...
                        await self._send_locked(
                            ...
                            error_frame(
                                "encryption_required",
                                "Logos requires encrypted frames on this session.",
                            ),
                        )
                        continue
                    ...
                    await self._send_locked(
                        websocket, meta, error_frame("protocol_error", str(exc))
                    )
                except Exception:
                    ...
                    await self._send_locked(
                        websocket,
                        meta,
                        error_frame("internal_error", "Logos adapter internal error."),
                    )
        finally:
            async with self._lock:
                self._clients.pop(websocket, None)
    ...
    def _authenticate(
        ...
        if raw_device_id is not None and raw_device_id != device_id:
            return (
                False,
                "invalid_device_id",
                "invalid Logos signed hello: device_id must not include surrounding whitespace",
                {},
                None,
            )
        if not device_id:
            return (
                False,
                "missing_device_id",
                "invalid Logos signed hello: device_id is required",
                {},
                None,
            )
        if payload_device_id and payload_device_id != device_id:
            return (
                False,
                "device_mismatch",
                "invalid Logos signed hello: payload device_id does not match frame device_id",
                {},
                None,
            )
        if "secret" in payload:
            return (
                ...
                "invalid Logos signed hello: plaintext secrets are not accepted",
                {},
                None,
            )
        signature_raw = payload.get("signature")
        nonce_raw = payload.get("nonce")
        # JSON wire value: coerced via int(...) inside the try/except below, which preserves the
        # explicit "must be an integer" rejection for non-numeric or missing payloads.
        timestamp_ms_raw: Any = payload.get("timestamp_ms")
        if not isinstance(signature_raw, str) or not signature_raw:
            return (
                False,
                "missing_signature",
                "invalid Logos signed hello: missing signature",
                {},
                None,
            )
        ...
        if len(nonce) < 12 or len(nonce) > 128:
            return (
                False,
                "invalid_nonce",
                "invalid Logos signed hello: nonce length is outside the allowed range",
                {},
                None,
            )
        try:
            ...
            return (
                False,
                "invalid_timestamp",
                "invalid Logos signed hello: timestamp_ms must be an integer",
                {},
                None,
            )
        ...
        if abs(now_ms - timestamp_ms) > MAX_HELLO_SKEW_SECONDS * 1000:
            return (
                False,
                "timestamp_skew",
                "invalid Logos signed hello: timestamp is outside the allowed clock skew",
                {
        ...
        if enc_client_nonce_raw is not None:
            if not isinstance(enc_client_nonce_raw, str) or not enc_client_nonce_raw:
                return (
                    False,
                    "invalid_enc_nonce",
                    "invalid Logos signed hello: enc_client_nonce must be a non-empty string",
                    {},
                    None,
                )
            try:
                ...
                return (
                    False,
                    "invalid_enc_nonce",
                    "invalid Logos signed hello: enc_client_nonce is not valid base64",
                    {},
                    None,
                )
            if len(enc_client_nonce_bytes) != ENC_NONCE_BYTES:
                return (
                    False,
                    "invalid_enc_nonce",
                    "invalid Logos signed hello: enc_client_nonce has an unexpected length",
                    {},
                    None,
                )
        ...
        if nonce in self._used_nonces:
            return (
                False,
                "replayed_nonce",
                "invalid Logos signed hello: nonce was already used",
                {},
                None,
            )
        credential_scope = "shared_master"
        ...
        if matched_secret is None:
            return (
                False,
                "invalid_signature",
                "invalid Logos signed hello: signature mismatch",
                {},
                None,
            )
    ...
    def _purge_nonces(self, *, now: float) -> None:
        stale = [
            nonce
            for nonce, seen_at in self._used_nonces.items()
